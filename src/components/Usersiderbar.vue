<template>
    ...
    <el-menu default-active="2" class="el-menu-vertical-demo" @open="handleOpen" @close="handleClose">
        ...
        <el-menu-item index="1" class="my-menu-item" @click="handleUploadDescImg">
            <el-icon>
                <setting />
            </el-icon>
            <span>上传描述图片</span>
        ...
        <el-menu-item index="2" class="my-menu-item" @click="handleUploadWatermarkImg">
            <el-icon>
                <setting />
            </el-icon>
            <span>上传水印图片</span>
    ...
    <el-dialog v-model="showDescImgDialog" title="上传描述图片" width="500" :before-close="closeDescImgDialog">
        <el-text>上传描述图片的内容</el-text>


        <el-row class='row-bg' justify='start'>
            <el-upload class="upload-demo" drag :action="uploadurl" :before-upload="isDesImgOK" :data="extraData"
                :on-success="uploadDescImg" multiple>
    ...
    <!-- 上传水印图片 -->
    <el-dialog v-model="showWatermarkImgDialog" title="上传水印图片" width="500" :before-close="closeWatermarkImgDialog">
        <el-text>上传水印图片的内容</el-text>

        <el-row class='row-bg' justify='start'>
            <el-upload class="upload-demo" drag :action="uploadurl" :before-upload="isWaterImgOK" :data="extraData"
                :on-success="uploadWatermarkImg" multiple>
                <el-icon class="el-icon--upload"><upload-filled /></el-icon>
                <div class="el-upload__text">
                    Drop file here or <em>click to upload</em>
                </div>
        ...
        <template #footer>
            <div class="dialog-footer">
                <el-button @click="showWatermarkImgDialog = false">Cancel</el-button>
            </div>
        </template>
    </el-dialog>
</template>
...
const uploadurl = "http://127.0.0.1:8080/waterimg/create";

const isDesImgOK = (file: File) => {
    //! 初始化 extraData
    if (tokenStore.getInfo().uid == "") {
        console.log("用户未登录，无法上传描述图片")
        extraData.value.user_id = "96271c58-df93-4589-9c3e-ef99a421e889"
    } else {
