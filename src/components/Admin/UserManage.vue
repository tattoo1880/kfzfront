<template>
    ...
    <el-row class="bg-row" justify="end" style="margin-right: 30px;">
        <el-button type="primary" :icon="CirclePlus" circle @click="makeaddture" />
    </el-row>
    ...
    <el-row class="row-bg" justify="center">
        <el-col :span="24">
            <el-table :data="tabledata" style="width: 100%;height: 70vh;" align="center">
                ...
                <el-table-column prop="title" label="用户名" width="150" align="center">
                    <template v-slot="{ row }">
                ...
                <el-table-column prop="authorname" label="电话" width="120" align="center">
                    <template v-slot="{ row }">
                        <el-tag type="success">{{ row.phone }}</el-tag>
                    </template>
                </el-table-column>

                <el-table-column prop="price" label="邮箱" width="100" align="center">
                    <template v-slot="{ row }">
                        <el-tag type="success">{{ row.email }}</el-tag>
                    </template>
                </el-table-column>

                <el-table-column prop="quality" label=" 是否有效" width="100" align="center">
                    <template v-slot="{ row }">
                        <!-- <el-tag type="success">{{ row.isActivated }}</el-tag> -->
                        <el-tag v-if="row.isActived" type="success">有效</el-tag>
                        <el-tag v-else type="danger">无效</el-tag>
                    </template>

                </el-table-column>

                <el-table-column prop="press" label="订阅开始时间" width="200" align="center">
                ...
                <el-table-column prop="pubDateText" label="订阅结束时间" width="200" align="center">
                    <template v-slot="{ row }">
                ...
                <el-table-column prop="link" label="操作" width="100" align="center">
                    <template v-slot="{ row }">
    ...
    <el-dialog title="修改用户" v-model="showv2" width="30%">
        <el-form :model="updateform" label-position="left" label-width="auto" class="withborder">
            ...
            <el-form-item label="订阅开始时间">
                ...
                <el-date-picker v-model="updateform.subStartTime" type="date" placeholder="Pick a day" size="small"
                    value-format="YYYY-MM-DD" />
            </el-form-item>
            <el-form-item label="订阅结束时间">
                <!-- <el-input v-model="updateform.subEndTime" /> -->
                <el-date-picker v-model="updateform.subEndTime" type="date" placeholder="Pick a day" size="small"
                    value-format="YYYY-MM-DD" />
            </el-form-item>
        </el-form>
        <template #footer>
...
const confirmadd = async () => {
    ...
    const res = await useusermanage.addUser(uid, username, password, phone, email);
    console.log(res);
    if (res.data !== '') {
